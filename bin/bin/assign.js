...
module.exports = {
  ...
  options: {
    users: { alias: 'u', value: 'username|email...', description: 'specify users to be assigned roles' },
  ...
  description: [
    ...
    'You may specify as many users and roles as you like, all users will be assinged every role.',
    'If you do not specify a role or a user you will be asked interactively to provide one.'
  ].join(" "),
  examples: [
    'assign -u user_a -r role_a',
    'assign -u user_a role_a role_b role_c',
    'assign role_a -u "user_a, user_b, user_c"'
  ],
        ...
        for(var user_index = 0; user_index < argv.users.length; user_index++){
          for(var role_index = 0; role_index < argv.roles.length; role_index++){
            try{
              ...
              ctx.stdout('assigned role: ' + result.role.name + " to user: " + result.user.username);
            } catch(err){
              ctx.stderr(err);
            }
          }
