...
module.exports = function revokeRole(obj){
  return function *revokeRole(){
    ...
    if(role.privileges.indexOf(privilege._id) < 0)
      throw new Error(role.name + ': does not have privilege: ' + privilege.name)
    role.privileges.splice(role.privileges.indexOf(privilege._id), 1);
